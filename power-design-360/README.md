...
- 可加入最多 6 個其他輸出檔位，依 3 個母線點與 5 個負載比例建立最多 105 列工況；以全部工況的電流、磁通與電容需求選值。表格可篩選、下載 CSV，控制模式不足處會標記。
- 可輸入已知的元件可用空間與散熱瓦數，排除磁件無法容納或目標損耗超出預算的候選；不需要猜測熱阻。
- 從 8 個 Panasonic ZU 真實輸出電容料號選出並聯組合，顯示數量、容差後容量、ESR、頻率修正後漣波能力、外形及官方來源。尚非完整 BOM。
...
| `lib/power/design-envelope.ts` / `component-catalog.ts` | 跨工況解析、磁件外形與實際輸出電容料庫 |
...
v0.5 自動設計使用公開的一階非對稱半橋／半波諧振模型，產生 HWLLC 設計起點，不是 RRW11011 已驗證的閉迴路模型。基礎來自 Infineon XDPS2201 設計指南、TI 兩相臨界導通 PFC 方法與 TDK 磁芯表；未將其他控制器的腳位設定套給 RRW11011。詳見 `docs/design-synthesis-v04.md` 與 `docs/design-envelope-v05.md`。

初始漏感比例 3%、負磁化電流係數 15%、電容負容差 20%、常溫設計 Bpk 0.18 T／J 3 A/mm²／銅填充 28% 都是可追溯的軟體策略。v0.5 使用固定理想 LS 諧振脈寬的連續脈衝工況，涵蓋所列輸出電壓、10／25／50／75／100% 負載與標稱／最高／保持終點母線，按最差應力選值。低壓大電流可能失去假設的負磁化電流，會顯示需控制模式驗證；保留候選不代表該工作點已可用。實際 RRW11011 變頻、變脈寬、跳週期及其他模式未模擬，無載、啟動、短路、檔位轉換、ZVS、磁芯損耗、製造容差與安規絕緣仍需驗證。

效率目標只分配前後級預算；熱模型回推需要的有效熱阻。已知散熱能力可用來反推最低效率，沒有把目標當作已達到的效率或溫升。TDK 磁件以含骨架的外形及必要面積篩選，未完成安規間距、母線電容、散熱器與佈線的整體配置。氣隙是忽略磁芯磁阻／邊緣效應的等效起點，不能當作加工圖。

輸出電容小型料庫使用 Panasonic ZU 2025-09-01 資料表：20% 負容量容差，20°C／100 kHz ESR 加上軟體 2 倍餘裕，135°C 漣波額定值依頻率修正再留 20% 均流餘裕；最多 24 顆同料並聯。這些條件未保證低溫、壽命末期或所有頻譜下的表現。4000 小時耐久試驗不是產品壽命。未查庫存、價格，也未完成所有元件的料號篩選。

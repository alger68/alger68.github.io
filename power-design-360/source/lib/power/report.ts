import { domains, limitations, sources } from './catalog.ts';
...
export function reportHtml(project: Project, result: Result) {
  const rows = Object.entries(result.metrics).map(([k,v]) => `<tr><td>${escape(k)}</td><td>${v===null?escape(k==='caseC'?'未提供有效熱阻':k==='bridgeConductionW'?(project.spec.rectifier==='diode'?'不適用：二極體橋':'未提供熱態 RDS(on)'):k==='boostDuty'?'不符合 Boost 工作點條件':'HWLLC：模型待建立'):escape(v.toFixed(4))}</td></tr>`).join('');
